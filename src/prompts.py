"""
60 cognitive task prompts from the paper's Appendix A (Tables 2 and 3).
6 categories x 10 prompts each.
"""
...
assert len(PROMPTS) == 60, f"Expected 60 prompts, got {len(PROMPTS)}"

# Map prompt index to category
def get_category(prompt_idx):
    """Return the category name for a given prompt index (0-59)."""
    return CATEGORIES[prompt_idx // 10]
